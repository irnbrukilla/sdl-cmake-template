...
int buffer_appendf(buffer_t *self, const char *format, ...)
{
    ...
    va_copy(tmpa, ap);
    int required = vsnprintf(NULL, 0, format, tmpa);
    va_end(tmpa);
    if (buffer_resize(self, length + required) == -1) {
        va_end(ap);
        return -1;
    }
    ...
    // have made room for.
    char *dst = self->data + length;
    int bytes = vsnprintf(dst, 1 + required, format, ap);
    va_end(ap);
...
/**
 * Return a new buffer based on the `from..to` slice of `buf`, or NULL on error.
 *
 * @param buf
 * @param from
 * @param to
